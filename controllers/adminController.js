...
const generatePDF = async (data, type) => {
  return new Promise((resolve) => {
    ...
    // Add content to PDF
    doc.fontSize(20).text('Braziconnect Portal System Data Export', { align: 'center' });
    doc.moveDown();
    ...
    // Add footer
    doc.moveDown(2);
    doc.fontSize(10).text('Generated by Braziconnect Portal System', { align: 'center' });
...
const getAdminProfile = async (req, res) => {
  try {
    ...
    if (!admin.profile) {
      const defaultProfile = await prisma.profile.create({
        data: {
          ...
          monthlyRate: '0',
          description: 'System Administrator for Braziconnect Portal',
    ...
    const adminProfile = {
      ...
      bio: admin.profile?.description || 'System Administrator for Braziconnect Portal',
...
const getSystemSettings = async (req, res) => {
  try {
    ...
    const settings = {
      ...
      sessionTimeout: 30,
      maxLoginAttempts: 5,
      systemName: 'Braziconnect Portal Admin',
    ...
    try {
      ...
      if (employerRequest.employerAccount?.user?.email) {
        ...
        await NotificationService.sendEmail({
          ...
          subject: 'Request Approved - Payment Required - Braziconnect Portal',
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <div style="background-color: #2c3e50; color: white; padding: 20px; text-align: center; border-radius: 0 0 8px 8px;">
                <p style="margin: 0; font-size: 12px; opacity: 0.8;">This is an automated notification from Braziconnect Portal. Please do not reply to this email.</p>
              </div>
            </div>
          `
        });
  ...
  try {
    ...
    if (employerRequest.employerAccount?.user?.email) {
      try {
        await NotificationService.sendEmail({
          to: employerRequest.employerAccount.user.email,
          subject: 'Payment Approved - Photo Access Granted - Braziconnect Portal',
          html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <!-- Header -->
          ...
          <div style="background-color: #2c3e50; color: white; padding: 20px; text-align: center;
            ...
            <p style="margin: 0; font-size: 12px; opacity: 0.8;">
              This is an automated notification from Braziconnect Portal. Please do not reply to this email.
            </p>
          </div>
        </div>
      `
  ...
  try {
    ...
    if (employerRequest.employerAccount?.user?.email) {
      try {
        await NotificationService.sendEmail({
          to: employerRequest.employerAccount.user.email,
          subject: 'Payment Rejected - Braziconnect Portal',
          html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <!-- Header -->
          <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  ...
  try {
    ...
    if (employerRequest.employerAccount?.user?.email) {
      try {
        await NotificationService.sendEmail({
          to: employerRequest.employerAccount.user.email,
          subject: 'Second Payment Approved - Full Access Granted - Braziconnect Portal',
          html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <!-- Header -->
          <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
          ...
          <div style="padding: 30px; background-color: #ffffff;">
            ...
            <div style="border-top: 2px solid #667eea; padding-top: 20px; margin-top: 30px;">
              <p>Best regards,</p>
              <div style="font-weight: bold; color: #2c3e50;">The Braziconnect Portal Team</div>
